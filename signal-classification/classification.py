...
# we won't use unnecessary fields
eeg.drop(
    [
        "indra_time",
        "Unnamed: 0",
...
eeg = eeg.loc[
    eeg["signal_quality"] < QUALITY_THRESHOLD
]  # filter the values based on a limit of 128
# the samples are from 0-128 (0 the best, 200 the worst)
print(eeg.shape)
# print(eeg.head())


# visualize one sample to understand what a signal is
...
#### Preprocess data
# replace labels from 67 to 19 instances
eeg.replace(
    {
        "label": {
            "blink1": "blink",
...
#### Calculate weights (somewhat complicated)
vals_dict = {}
for i in eeg["label"]:
...
optimizer = keras.optimizers.legacy.Adam(amsgrad=True, learning_rate=0.001) # normal adam is supposed to run slow on macs so use the legacy 
loss = keras.losses.CategoricalCrossentropy()


#### Compile the model and make it training-ready
conv_model.compile(
    optimizer=optimizer,
    loss=loss,
    metrics=[
